...
SYSTEM_PROMPT = (
    "You are a security code repair assistant.\n"
    "Given a short code snippet and a possible vulnerability description,\n"
    "output ONLY the corrected code with a simple, minimal fix.\n"
    "- No explanations.\n"
    "- No markdown or fences.\n"
    "- Keep the structure unchanged except for the fix.\n"
)
...
def load_seen_sft_names():
    ...
    seen_file = config.get_raw_file_path("defect_detection_sft_seen.jsonl")
    seen_names = set()
...
def process_defect_detection_to_sft_batch(llm, num_samples=None, batch_size=4):
    """Process Defect Detection data into SFT format using Qwen model with batch processing"""

    # Load raw data
    raw_file = config.get_raw_file_path("defect_detection_raw.jsonl")
    ...
    with open(raw_file, 'r', encoding='utf-8') as f:
        for i, line in enumerate(f):
            try:
                ...
                data['name'] = f"defect_detection_{i+1}"
    ...
    # Output files
    sft_file = config.get_raw_file_path("defect_detection_sft.jsonl")
    seen_file = config.get_raw_file_path("defect_detection_sft_seen.jsonl")
    ...
    for i, (sample, output) in enumerate(tqdm(
        ...
        desc="Generating defect analysis"
    )):
        try:
            ...
            sft_entry = {
                ...
                "name": sample.get("name", f"defect_detection_{i}")
            }
...
def main():
    ...
    count = process_defect_detection_to_sft_batch(llm, num_samples=None, batch_size=12)
